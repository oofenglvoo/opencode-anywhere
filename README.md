...
- 列表列宽自适应该列的表头与内容：内容不会被截断，筛选/刷新时列宽不跳动，列表随窗口宽度伸展
- 按标题、完整路径和目录名搜索
...
- 工作目录页：列出会话用过的工作目录及其大小、最后修改时间，一键打开/开终端/启动 opencode
...
> 打包前请先关闭正在运行的 GUI，否则会因 `PermissionError: [WinError 5] 拒绝访问 dist\opencode-anywhere.exe` 而失败。
...
- **搜索**：匹配会话标题、完整路径和目录名
- **目录**：只显示指定工作目录的会话；下拉框显示成「目录名（上级目录）」以便区分同名目录
- **状态**：筛选 `空闲`、`近期活跃` 或 `运行中`
- **列顺序**：`状态` / `消息` / `大小` / `标题` / `目录` / `更新时间` / `路径`
- **排序**：点击任意表头即按其排序，表头显示 `▲`(升序)/`▼`(降序)；同一列反复点切换升降序，换列时默认降序（`状态`/`标题`/`目录` 默认升序）。默认按 `更新时间` 倒序
- **大小**：每个会话的占用估算（含子会话的 message/part/event 数据），底部状态栏显示会话合计
- **目录 / 更新时间 / 路径**：`目录` 列只显示最后一级文件夹名称，中间是 `更新时间`，`路径` 列显示完整路径
- **列宽**：`状态`/`消息`/`大小`/`标题`/`目录`/`更新时间` 按表头与实际内容里更宽的那个取宽，保证内容不被截断；超长的标题/目录名设上限，把剩余宽度全部留给 `路径`（`路径` 是唯一随窗口伸展的列）
...
- **F5**：刷新当前页（会话管理→重载会话，会话同步→刷新远端会话包，工作目录→重新统计）
...
列出各会话用过的工作目录（按 `session.directory` 归并），一屏看清哪些目录还在用、各自多大、最后一次改动是什么时候。

- **列**：`目录名称` / `会话`（该目录下的会话数）/ `最后修改`（递归最新文件时间）/ `大小`（递归总大小）/ `绝对路径`；点击表头可切换排序，默认按最后修改倒序
- **双击**：在资源管理器中打开该目录
- **右键**：资源管理器中打开 / 启动 opencode（在该目录新开控制台）/ 开终端 / 复制完整路径 / 在会话管理中筛选（跳到会话管理并预置目录筛选）
- **搜索**：匹配目录名和完整路径
- 磁盘上已不存在的目录会被隐藏，只在底部状态栏计数
- 大小与最后修改时间由后台线程递归统计（`os.scandir` 实现，约 30 个目录 0.4 秒），加载时先出目录清单再补数字
- 列宽按表头与内容自适应（`目录名称`/`最后修改`/`大小` 取表头与全量内容里更宽的那个），不随筛选跳动
- `绝对路径` 是唯一随窗口伸展的列，拉伸窗口时列表会跟着变宽/变窄
- **F5** 重新统计（与「刷新」按钮等价）
- 底部提示条（`hint`）在 `tree` **之前**打包：早期版本把它放在后面，超长单行文本会把列表宽度钉死在列宽之和，导致窗口拉伸时列表不跟着变宽
...
- 工作目录（目录名/上级目录简称、递归大小与文件数、缺失目录容错、会话目录归并与隐藏缺失、列宽按表头自适应）
- 内容列宽与布局（按内容取宽/无样本回退表头/`caps` 限制列宽；工作目录页 `hint` 必须排在 `tree` 之前，否则窗口拉伸时列表不跟随）
...
### 列表内容显示不全，或拉伸窗口时列表不跟着变宽

当前版本已修：

- 每列的宽度取「表头」与「该列全部内容」里更宽的那个（不是只看表头），所以状态、消息、大小、标题、目录名、时间都不会被截断
- 样本取自全量数据而不是当前可见行，所以搜索/筛选/刷新时列宽不会来回跳
- 超长的标题/目录名会设上限，把剩余宽度留给唯一伸展的 `路径` / `绝对路径` 列
- 工作目录页的底部提示条改为在列表**之前**打包并加了自动换行 —— 之前它排在列表之后且是一整行超长文本，会把列表宽度钉死在约 100px，窗口再大也不变

如果界面仍是旧样子，确认启动的是最新代码或重新打包的 `dist\opencode-anywhere.exe`（`tools\ocp-gui.cmd` 会优先用它）。
