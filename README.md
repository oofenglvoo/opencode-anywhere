...
- **目录 / 更新时间 / 路径**：`目录` 列只显示最后一级文件夹名称，中间是 `更新时间`，`路径` 列显示完整路径（该列随窗口伸展）
- **列宽**：`状态`/`消息`/`大小`/`标题`/`目录`/`更新时间` 按表头与实际内容里更宽的那个取宽，保证内容不被截断；超长的标题/目录名会设上限，把剩余宽度留给 `路径`
...
- 列宽按表头与内容自适应（`目录名称`/`最后修改`/`大小` 取表头与全量内容里更宽的那个），不随筛选跳动；列表随窗口宽度伸展
